...
const LENDING_CONTRACT_ADDRESS =
  process.env.NEXT_PUBLIC_LENDING_CONTRACT_ADDRESS || "";
...
function WalletStateController({ children }: { children: ReactNode }) {
  ...
  // Deposit collateral function using XellarSDK
  const depositCollateral = async (tokenAddress: string, amount: string) => {
    ...
    try {
      // 1. ensure user is logged in to Xellar
      await config.auth.socialLogin("google");

      // 2. approve token
      const approveData = config.contract.encodeFunctionData({
        contractAddress: tokenAddress,
        abi: [
          "function approve(address spender, uint256 amount) public returns (bool)",
        ],
        functionName: "approve",
        params: [LENDING_CONTRACT_ADDRESS, amount],
      });
      const approveTx = await config.wallet.sendTransaction({
        to: tokenAddress,
        data: approveData,
        value: "0",
      });
      ...
      // 3. depositCollateral
      const depositData = config.contract.encodeFunctionData({
        contractAddress: LENDING_CONTRACT_ADDRESS,
        abi: LENDING_CONTRACT_ABI,
        functionName: "depositCollateral",
        params: [tokenAddress, amount],
      });
      const depositTx = await config.wallet.sendTransaction({
      ...
      toast({
        ...
        description: "Could not deposit collateral",
      });
    }
  };
