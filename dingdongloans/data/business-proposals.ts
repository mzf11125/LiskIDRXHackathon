...
export const getBusinessProposalById = async (id: string): Promise<BusinessProposal | null> => {
    try {
        ...
        return {
            ...data,
            wallet_analysis: getAIWalletAnalysis(data.proposer_wallet)
        };
    } catch (error) {
        ...
        if (proposal) {
            // Ensure wallet analysis is included in mock data
            return {
                ...proposal,
                wallet_analysis: getAIWalletAnalysis(proposal.proposer_wallet)
            };
        }
