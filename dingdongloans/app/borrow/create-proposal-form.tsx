...
export default function CreateProposalForm({
	...
	async function onSubmit(values: FormValues) {
		const proposal: Partial<BusinessProposal> = {
			...values,
			deadline: values.deadline.toISOString(), // Convert Date to string
			status: "active",
	...
	const removeTag = (tag: string) => {
		...
		<div className="max-h-[calc(80vh-120px)] overflow-y-auto pr-2 
				[scrollbar-width:thin] [scrollbar-color:rgba(100,116,139,0.5)_transparent] 
				[&::-webkit-scrollbar]:w-[6px] [&::-webkit-scrollbar-track]:bg-transparent 
				[&::-webkit-scrollbar-thumb]:bg-[rgba(100,116,139,0.5)] [&::-webkit-scrollbar-thumb]:rounded-[20px]
				[&::-webkit-scrollbar-thumb:hover]:bg-[rgba(100,116,139,0.7)]">
			<Form {...form}>
				<form
