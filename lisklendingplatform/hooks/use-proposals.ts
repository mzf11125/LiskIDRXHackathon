...
export function useProposals(poolId?: string) {
    ...
    async (id?: string) => {
      ...
      try {
        ...
        const response = await api.get<Proposal[]>(
          `/api/pools/${targetPoolId}/proposals`
        );
  ...
  const fetchMyProposals = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // Since there's no specific endpoint for user proposals,
      // we'll return an empty array for now.
      // The wallet analysis can be fetched from /wallets/${address}
      // if needed for risk assessment or other features
      console.warn(
        "No endpoint exists for fetching user proposals. Returning empty array."
      );
      setMyProposals([]);
      return [];
    } catch (error) {
  ...
  const createProposal = useCallback(
    async (data: NewProposal) => {
      ...
      try {
        ...
        const response = await api.post<typeof proposalData, Proposal>(
          "/api/proposals",
          proposalData
        );
  ...
  const voteOnProposal = useCallback(
    async (proposalId: string, voteFor: boolean) => {
      if (!isConnected || !address) {
        toast({
          variant: "destructive",
          title: "Error",
      ...
      try {
        ...
        const response = await api.post<typeof voteData, any>(
          "/api/proposals/vote",
          voteData
        );
